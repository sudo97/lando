...
test("Applicative", () => {
  ...
  defaultAssert(
    ...
    Ok("a"),
    ~operator="pure",
    ~message="pure of ParseResult will return Ok(string)",
  )

  let sum = (a, b) => a + b

  defaultAssert(
    pure(sum)->apply(pure(1))->apply(pure(2)),
    Ok(3),
    ~operator="apply",
    ~message="apply should apply function to value",
  )
  ...
  defaultAssert(
    ...
    Ok([1, 2]),
    ~operator="sequence",
    ~message="sequence should return Ok(array) if all elements are Ok",
  )
...
test("computeNextTarget", () => {
  [
    ...
    (
      `{"protocols":["prioritize-mech"],"scan":[{"coordinates":{"x":0,"y":40},"enemies":{"type":"soldier","number":10}},{"coordinates":{"x":0,"y":80},"allies":5,"enemies":{"type":"mech","number":1}}]}`,
      `{"x":0,"y":80}`,
    ),
    (
    ...
    (
      `{"protocols":["assist-allies"],"scan":[{"enemies":{"number":10,"type":"soldier"},"allies":3,"coordinates":{"y":35,"x":5}},{"enemies":{"number":20,"type":"soldier"},"coordinates":{"y":5,"x":35}}]}`,
      `{"x":5,"y":35}`,
    ),
    (
      `{"protocols":["avoid-crossfire"],"scan":[{"enemies":{"number":10,"type":"soldier"},"allies":3,"coordinates":{"y":35,"x":5}},{"enemies":{"number":20,"type":"soldier"},"coordinates":{"y":5,"x":35}}]}`,
      `{"x":35,"y":5}`,
    ),
    (
      `{"protocols":["furthest-enemies"],"scan":[{"enemies":{"number":10,"type":"soldier"},"coordinates":{"y":35,"x":5}},{"enemies":{"number":20,"type":"soldier"},"coordinates":{"y":30,"x":10}}]}`,
    ...
    (
      `{"protocols":["closest-enemies","avoid-mech"],"scan":[{"coordinates":{"x":0,"y":1},"enemies":{"type":"mech","number":1}},{"coordinates":{"x":0,"y":10},"enemies":{"type":"soldier","number":10}},{"coordinates":{"x":0,"y":99},"enemies":{"type":"mech","number":1}}]}`,
      `{"x":0,"y":10}`,
    ),
    (
    ...
    defaultAssert(
      parsedInp,
      parsedOutp,
      ~operator="computeNextTarget",
      ~message=`computeNextTarget ${inp} => ${outp}`,
    )
  })
})
